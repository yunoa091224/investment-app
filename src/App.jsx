...
const STEP1_PROMPT = `あなたは世界トップクラスのファンダメンタルアナリストです。指定銘柄について以下の各項目を具体的に分析し、箇条書きで回答してください（JSON不要・日本語・500字以内）。

【事業モデル・競合優位性】
・主力事業と収益構造（何で稼いでいるか）
・業界内の市場シェアと競合他社との差別化要因
・参入障壁・ブランド・特許・ネットワーク効果など経済的堀の有無

【財務指標評価】
・PER・PBR・ROEの水準と業界平均との比較
・直近の売上成長率・営業利益率・フリーキャッシュフロー
・バランスシートの健全性（負債比率・自己資本比率）

【成長ドライバーと業界トレンド】
・業界全体の成長トレンドと同社が恩恵を受ける理由
・今後12〜18ヶ月の主要触媒（新製品・規制変化・地政学的要因など）

【強み・弱みまとめ】
・強み（箇条書き2〜3点）
・弱み・リスク（箇条書き2〜3点）`;

const STEP2_PROMPT = `あなたは世界トップクラスのテクニカルアナリストです。指定銘柄について以下の各項目を具体的に分析し、箇条書きで回答してください（JSON不要・日本語・500字以内）。

【トレンド判定】
・直近トレンドを「上昇・下降・横ばい」で明確に判定し根拠を述べる
・週足・日足それぞれの方向感

【モメンタム指標】
・RSI（14日）の現在水準：過買い(70超)・中立・過売り(30未満)のどれか、反転リスクの有無
・MACD：ゴールデンクロス/デッドクロスの状況、ヒストグラムの方向
・移動平均線：50日MAと200日MAの位置関係（ゴールデンクロス/デッドクロス/価格との乖離率目安）

【ボリンジャーバンド】
・現在の価格位置（上限・中央・下限のどの付近か）
・バンド収縮/拡大の状況と示唆（ブレイクアウト前兆か否か）
...
async function callAPIRaw(systemPrompt, userMessage) {
  const res = await fetch("https://api.anthropic.com/v1/messages", {
    method:"POST",
    headers:{ "Content-Type":"application/json", "x-api-key":import.meta.env.VITE_ANTHROPIC_API_KEY, "anthropic-version":"2023-06-01", "anthropic-dangerous-direct-browser-access":"true" },
    body: JSON.stringify({ model:"claude-haiku-4-5-20251001", max_tokens:600, system:systemPrompt, messages:[{ role:"user", content:userMessage }] }),
  });
  const json = await res.json();
  if (json.error) throw new Error(json.error.message);
  const tb = json.content?.find(b=>b.type==="text");
...
function StockCard({ stock, color, expanded, onToggle, marketOpen }) {
  ...
  const priceLabel = stock._realPrice ? (marketOpen ? "RT" : "前日終値") : "AI推定";
  const priceBadgeBg    = stock._realPrice ? (marketOpen ? "#00c9ff22" : "#ffd70018") : "#ffffff08";
  const priceBadgeColor = stock._realPrice ? (marketOpen ? "#00c9ff"   : "#ffd700")   : "#7a90a8";
  const priceBadgeBorder= stock._realPrice ? (marketOpen ? "#00c9ff33" : "#ffd70035") : "#0d2030";
  return (
        ...
        <div style={{ textAlign:"right", flexShrink:0 }}>
          <div style={{ display:"flex", alignItems:"center", gap:4, justifyContent:"flex-end" }}>
            ...
            <span style={{ fontSize:8, padding:"1px 4px",
              ...
              borderRadius:3, border:`1px solid ${priceBadgeBorder}` }}>
              {priceLabel}
            </span>
          </div>
      ...
      <div style={{ borderTop:"1px solid #0d2030", padding:"16px 16px 8px" }}>
        ...
        {d&&!d.error&&!isLoading&&(
          <div style={{ animation:"fadeIn .4s ease" }}>
            {/* 市場ステータスバー */}
            <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:10, padding:"6px 10px", background:"#06111a", borderRadius:8, border:"1px solid #0d2030" }}>
    ...
    <div style={{ padding: "16px 16px 100px" }}>
      ...
      <div style={card}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginBottom: 10 }}>
          ...
          <div>
            <label style={lbl}>目標株価（{mode === "jp" ? "JPY 円" : "USD"}）</label>
            <input style={input} type="number" placeholder={mode === "jp" ? "3000" : "1000"} value={targetPrice}
      ...
      {/* Status */}
      {status && (
        <div style={{ padding: "10px 14px", background: `${status.color}18`, border: `1px solid ${status.color}55`, borderRadius: 8, color: status.color, fontSize: 13, marginBottom: 12 }}>
          {status.msg}
        </div>
      )}
    ...
    <ErrorBoundary>
      ...
      <div style={{ minHeight:"100vh", background:C.bg, fontFamily:"'Courier New', monospace", color:"#c8d8e8", paddingBottom:72 }}>
        ...
        <nav className="tabnav" style={{ position:"fixed", bottom:0, left:0, right:0, background:"#060e18", borderTop:"1px solid #0d2030", display:"flex", zIndex:50, overflowX:"auto", scrollbarWidth:"none", WebkitOverflowScrolling:"touch" }}>
          {TABS.map(t=>(
            <button key={t.key} onClick={()=>{ setActiveTab(t.key); setTabTip(null); }} style={{ minWidth:68, flexShrink:0, padding:"10px 4px 8px", background:"transparent", border:"none", borderTop:activeTab===t.key?"2px solid #00ffcc":"2px solid transparent", color:activeTab===t.key?"#00ffcc":"#cccccc", cursor:"pointer", fontFamily:"inherit", display:"flex", flexDirection:"column", alignItems:"center", gap:3, position:"relative" }}>
              {t.img
                ? <img src={t.img} alt={t.label} style={{ width:24, height:24, objectFit:"contain", opacity:activeTab===t.key?1:0.85 }}/>
                : <span style={{ fontSize:18, opacity:activeTab===t.key?1:0.85 }}>{t.icon}</span>
              }
