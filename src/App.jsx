...
const STEP1_PROMPT = `あなたは世界トップクラスの株式アナリストです。指定銘柄のファンダメンタル分析を行ってください。企業概要・主力事業・財務状況（売上成長率・収益性・バリュエーション目安）・業界内の競合優位性・主なリスク要因を300字以内で箇条書きにしてください。JSON不要・日本語で回答。`;

const STEP2_PROMPT = `あなたは世界トップクラスのテクニカルアナリストです。指定銘柄のテクニカル分析を行ってください。現在のトレンド方向・RSI水準と過熱感・移動平均線（50日・200日）との位置関係・ボリンジャーバンドの状況を300字以内で箇条書きにしてください。JSON不要・日本語で回答。`;
...
async function callThreeStepAnalysis(ticker, onStep) {
  ...
  const result = await callAPI(
    ANALYSIS_PROMPT,
    `銘柄: ${ticker}\n\n【Step1 ファンダメンタル分析】\n${fundamental}\n\n【Step2 テクニカル分析】\n${technical}\n\n上記の分析を統合して総合判断してください。JSONのみ返してください。`
  );
  return result;
}
